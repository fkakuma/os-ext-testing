...
PROJECT_CONFIG_REPO=${PROJECT_CONFIG_REPO:-https://review.openstack.org/p/openstack-infra/project-config.git}
...
PUPPET_MODULE_PATH="--modulepath=$OSEXT_PATH/puppet/modules:/root/config/modules:/root/project-config/nodepool:/etc/puppet/modules"
...
if [[ ! -e install_puppet.sh ]]; then
  ...
  sudo git clone $PROJECT_CONFIG_REPO /root/project-config
  sudo cp -r /root/project-config/nodepool/scripts /opt/nodepool-scripts
